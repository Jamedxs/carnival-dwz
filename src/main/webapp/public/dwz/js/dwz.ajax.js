...
xianghua
*/
function ourAjaxDone(json){
    DWZ.ajaxDone(json);
    if (json.statusCode == DWZ.statusCode.ok){
        if(json.navTabId){
            if (json.navTabId!="current"){                              
                navTab.reload(json.forwardUrl, {
        ...
        if(json.closedialog)
        {
            if(json.closedialog=="current")
            {
                $.pdialog.closeCurrent();
            }else{
                $.pdialog.close(json.closedialog);
            }
        }
        if(json.reload_p){
            $.pdialog.reload_p(json.reload_p);
        }
    }
}
/**
...
function dialogAjaxDone(json){	
	...
	if (json.statusCode == DWZ.statusCode.ok){		
		...
		} else if(!json.dialogPageRel) {			
			var $pagerForm = $("#pagerForm", navTab.getCurrentPanel());
			var args = $pagerForm.size()>0 ? $pagerForm.serializeArray() : {}
			navTabPageBreak(args, json.rel);
		}	
		if(!json.dialogPageRel||$.pdialog._current.find("#"+json.dialogPageRel).size()==0){
			$.pdialog.closeCurrent();
		}
		...
		if(json.dialogPageRel){			
			var $pagerForm = $("#pagerForm", $("#"+json.dialogPageRel));
			divSearch($pagerForm, json.dialogPageRel);
		}
...
function dialogAjaxDonePenoncel(json){	
//	DWZ.ajaxDone(json);
	if (json.statusCode == DWZ.statusCode.ok){		
		if(!json.dialogPageRel||$.pdialog._current.find("#"+json.dialogPageRel).size()==0){
			alertMsg.ensure("密码修改成功,请重新登录!");
			$.pdialog.closeCurrent();
		}
