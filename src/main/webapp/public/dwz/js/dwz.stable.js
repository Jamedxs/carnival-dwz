		...
		return this.each(function(){
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(){
					...
					if (sTarget) {
						//insert start (2013-06-24 不不)
						 for (var i=0; i < $ftds.size(); i++) {   //add by myself
							var $ftd = $($ftds[i]);
							var tdTarget = $ftd.attr("target");
							if (tdTarget){
								if ($("#"+tdTarget, $grid).size() == 0) {
									$grid.prepend('<input id="'+tdTarget+'" type="hidden" />');
								}
								$("#"+tdTarget, $grid).val($ftd.attr("rel"));
							}else{
								break;
							}
						}
			...
			}).dblclick(function(){$(this).parents(".pageContent").find("a[rowclick='true']").click()});  //update  (2013-06-23 不不) 原本此行为 }); 添加了dbclick事件
